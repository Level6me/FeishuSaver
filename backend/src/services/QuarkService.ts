...
export class QuarkService implements ICloudStorageService {
  ...
  async setCookie(req: Request): Promise<void> {
    ...
    } else if (process.env.QUARK_COOKIE) {
      this.cookie = process.env.QUARK_COOKIE;
    } else {
  ...
  async getShareInfoDeep(pwdId: string, passcode = "", page: number = 1): Promise<ShareInfoResponse> {
    const response = await this.api.post(
      `/1/clouddrive/share/sharepage/token?pr=ucpro&fr=pc&uc_param_str=&__dt=994&__t=${Date.now()}`,
    ...
    if (response.data?.status === 200 && response.data.data) {
      const fileInfo = response.data.data;
      if (fileInfo.stoken) {
        const res = await this.getShareListDeep(pwdId, fileInfo.stoken, page);
        return { data: res };
      }
    }
    throw new Error("获取夸克分享信息失败");
  ...
  async checkShareValid(pwdId: string): Promise<boolean> {
    try {
      const response = await this.api.post(
        `/1/clouddrive/share/sharepage/token?pr=ucpro&fr=pc&uc_param_str=&__dt=994&__t=${Date.now()}`,
        { pwd_id: pwdId, passcode: "" }
      );
      return response.data?.status === 200 && !!response.data.data?.stoken;
    } catch (e) {
      return false;
    }
  }

  async getShareList(pwdId: string, stoken: string, page: number = 1): Promise<ShareInfoResponse["data"]> {
    ...
    if (response.data?.data) {
      ...
      return {
        list,
        pwdId: response.data.data.share.pwd_id,
        stoken: stoken,
      };
    } else {
      return {
  ...
  async getShareListDeep(pwdId: string, stoken: string, page: number = 1): Promise<ShareInfoResponse["data"]> {
    const rootData = await this.getShareList(pwdId, stoken, page);
    const response = await this.api.get("/1/clouddrive/share/sharepage/detail", {
      params: {
        pr: "ucpro",
        fr: "pc",
    ...
    if (response.data?.data) {
      const rootList = response.data.data.list;
      if (rootList && rootList.length === 1 && rootList[0].file_type === 0) {
        const fid = rootList[0].fid;
        const subResponse = await this.api.get("/1/clouddrive/share/sharepage/detail", {
          params: { pwd_id: pwdId, stoken: stoken, pdir_fid: fid, _page: page.toString(), _size: "50", __t: Date.now() },
        });
        if (subResponse.data?.data) {
          const subList = subResponse.data.data.list
            .filter((item: any) => item.fid)
            .map((item: any) => ({
              fileId: item.fid,
          ...
          return { list: subList, pwdId, stoken, fileSize: subResponse.data.data.share?.size || 0 };
        }
      }
    }
    return rootData;
