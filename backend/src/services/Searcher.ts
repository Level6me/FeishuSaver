      ...
      try {
        ...
        return this.searchInWeb(url).then((results) => {
          ...
          if (results.items.length > 0) {
            ...
            const extractCastText = (content: string): string => {
              if (!content) return '';
              const lines = content.split('\n');
              const castParts: string[] = [];
              for (const line of lines) {
                const trimmed = line.trim();
                if (/^(?:主演|导演|演员|配音|制作)\s*[:：]/.test(trimmed)) {
                  castParts.push(trimmed);
                  continue;
                }
                const matches = trimmed.match(/[（(][^）)]*?(?:饰|配音)[）)]/g);
                if (matches) {
                  castParts.push(...matches);
                }
              }
              return castParts.join(' ');
            };

            const normalizedTerms = keyword 
              ? keyword.split(/\s+/).map(t => normalizeText(t)).filter(t => t.length > 0) 
              : [];

            const channelResults = results.items
              .filter((item: sourceItem) => item.cloudLinks && item.cloudLinks.length > 0)
              .filter((item: sourceItem) => {
                if (normalizedTerms.length === 0) return true;
                
                const title = item.title || '';
                const tags = item.tags?.join(' ') || '';
                const castText = extractCastText(item.content || '');
                
                const rawSearchArea = `${title} ${castText} ${tags}`;
                const searchArea = normalizeText(rawSearchArea);
                
                return normalizedTerms.every(term => searchArea.includes(term));
              })
              .map((item: sourceItem) => ({
                ...item,
